...
interface Project {
  ...
  blockchainProjectId?: string; // Added for onchain project ID
}
...
export default function AllProjectsPage() {
  ...
  const fetchUserTeam = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (teams.length > 0) {
          setUserTeam(teams[0]); // Use the first team for now
        }
      }
    } catch (error) {
  ...
  const getApplicationStatus = (project: Project, team: any) => {
    if (!team) return 'Join a Team to Apply';
  ...
  const isApplicationReady = (project: Project, team: any) => {
  ...
  const handleApply = (project: Project) => {
    if (!userTeam) {
      alert('You need to be part of a team to apply for projects. Please create or join a team first.');
      return;
    }
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      const onchainTeamId = userTeam.onchainTeamId;
              ...
              >
                ...
                <div className="p-6 space-y-4">
                  ...
                  <button
                    ...
                    disabled={!isApplicationReady(project, userTeam)}
                    className={`w-full mt-4 px-4 py-2 rounded-lg transition-colors text-center font-medium flex items-center justify-center gap-2 ${
                      isApplicationReady(project, userTeam)
                  ...
                  >
                    ...
                    {loadingTeam 
                      ...
                      : project.status === 'HIRING' 
                        ? getApplicationStatus(project, userTeam)
                        : 'Not Hiring'
                    }
                  </button>
                </div>
